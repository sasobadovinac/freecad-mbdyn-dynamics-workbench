...
###################################################################################
'''
This class receives a non-parametric body and creates a structural static node positioned at the body's center of mass.
A structural static node is an owner of kinematic degrees of freedom. 
...
Note that static nodes are not supposed to have initial velocity or angular velocity?. In general, a clamp joint will be used to "clamp" the static node to the gobal inertial frame. 
The static node can then be used to apply, for instance, an axial rotation joint to rotate a dynamic node.           
'''
...
class Structuralstaticnode: 
    def __init__(self, obj, baseBody):  
        
        #create a node at the absolute possition of the center of mass of the CAD object:
        ...
        #Initial absolute orienation (Euler angles):  
        ...
        obj.addProperty("App::PropertyString","type","Structural dynamic node","type",1).type = 'static'
        ...
        obj.addProperty("App::PropertySpeed","velocity_X","Initial absolute velocity","velocity_X",1).velocity_X = vx
        obj.addProperty("App::PropertySpeed","velocity_Y","Initial absolute velocity","velocity_Y",1).velocity_Y = vy
        obj.addProperty("App::PropertySpeed","velocity_Z","Initial absolute velocity","velocity_Z",1).velocity_Z = vz
        
        obj.addProperty("App::PropertyString","angular velocity X","Initial absolute angular velocity","angular velocity X",1).angular_velocity_X = str(wx)
        obj.addProperty("App::PropertyString","angular velocity Y","Initial absolute angular velocity","angular velocity Y",1).angular_velocity_Y = str(wy)
        obj.addProperty("App::PropertyString","angular velocity Z","Initial absolute angular velocity","angular velocity Z",1).angular_velocity_Z = str(wz)
    ...
    def onChanged(self, fp, prop):
        ...
        try:
            ...
            FreeCAD.ActiveDocument.getObjectsByLabel("x: structural: "+FreeCADGui.Selection.getSelection()[0].Label[-1])[0].Placement=FreeCAD.Placement(FreeCAD.Vector(x,y,z), FreeCAD.Rotation(yaw,pitch,roll), FreeCAD.Vector(0,0,0))
            FreeCAD.ActiveDocument.getObjectsByLabel("y: structural: "+FreeCADGui.Selection.getSelection()[0].Label[-1])[0].Placement=FreeCAD.Placement(FreeCAD.Vector(x,y,z), FreeCAD.Rotation(yaw,pitch,roll), FreeCAD.Vector(0,0,0))
            FreeCAD.ActiveDocument.getObjectsByLabel("z: structural: "+FreeCADGui.Selection.getSelection()[0].Label[-1])[0].Placement=FreeCAD.Placement(FreeCAD.Vector(x,y,z), FreeCAD.Rotation(yaw,pitch,roll), FreeCAD.Vector(0,0,0))
            FreeCAD.ActiveDocument.getObjectsByLabel("i: structural: "+FreeCADGui.Selection.getSelection()[0].Label[-1])[0].Placement=FreeCAD.Placement(FreeCAD.Vector(x,y,z), FreeCAD.Rotation(yaw,pitch,roll), FreeCAD.Vector(0,0,0))        
        except:
            return True
